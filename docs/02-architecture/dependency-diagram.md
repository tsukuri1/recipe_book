...
Такое разделение позволяет подменять `CachedRecipeRepository` на `FakeRecipeRepository` без переписывания экранов. UI продолжает работать с тем же ViewModel и Interactor, а источник данных меняется внутри Foundation-слоя.
