...
| `submitComplaint` | Отправляет жалобу пользователя на выбранный рецепт |
...
| `submitComplaint(recipeId, title, description)` | `RecipeViewModel` | ID рецепта, причина, описание | жалоба отправлена администратору |
...
| `create` | `ComplaintController` | принимает пользовательскую жалобу на рецепт |
