...
| POST | `/complaints` | USER | Отправить жалобу на рецепт |
...
| `presentation/screens/RecipeDetailScreen.kt` | Карточка рецепта, покупки, избранное, жалоба |
| `presentation/screens/SettingsScreen.kt` | Настройки |
| `presentation/screens/AdminScreen.kt` | Админ-панель |

Для эмулятора Android адрес Docker backend должен быть `http://10.0.2.2:8082/api/`, а не `localhost`, потому что `localhost` внутри эмулятора указывает на сам эмулятор. При запуске backend без Docker на порту `8080` можно использовать `http://10.0.2.2:8080/api/`.
