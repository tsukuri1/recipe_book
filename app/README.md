...
| REST API | 27 методов в `RecipeApi` |
...
| Жалоба на рецепт | Диалог в `RecipeDetailScreen` + `POST /api/complaints` |
...
- `RecipeDetailScreen` — ингредиенты, шаги, добавление в покупки, избранное и отправка жалобы.
...
Основной запуск приложения использует `CachedRecipeRepository`: он создается в `LadushkiApplication` через Retrofit, Room и `TokenStore`. `FakeRecipeRepository` оставлен как изолированная демо-реализация для тестирования UI без сервера.
