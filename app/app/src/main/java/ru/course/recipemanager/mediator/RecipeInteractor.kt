...
class RecipeInteractor(
    ...
    suspend fun addIngredientsToShoppingList(recipeId: Long) {
        ...
        repository.addIngredientsToShoppingList(recipeId, recipe.ingredients)
    }

    suspend fun submitComplaint(recipeId: Long, title: String, description: String) {
        val recipe: Recipe = repository.getRecipeById(recipeId) ?: return
        repository.submitComplaint(recipe.id, recipe.title, title, description)
    }
}
