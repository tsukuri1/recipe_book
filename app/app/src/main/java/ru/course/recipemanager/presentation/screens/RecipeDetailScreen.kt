...
fun RecipeDetailScreen(
    ...
    onSendComplaint: (String, String) -> Unit,
    onEdit: () -> Unit,
    onMessageShown: () -> Unit
) {
    ...
    Column(
        ...
        Card(
            ...
            Column(
                ...
                Button(
                    onClick = { complaintDialogOpen = true },
                    modifier = Modifier.fillMaxWidth(),
                    shape = RoundedCornerShape(18.dp),
                    colors = ButtonDefaults.buttonColors(containerColor = Rose, contentColor = BerryDark)
                ) {
                    Text("Пожаловаться на рецепт", fontWeight = FontWeight.ExtraBold)
                }
                Button(
                    onClick = onEdit,
                    modifier = Modifier.fillMaxWidth(),
    ...
    if (complaintDialogOpen) {
        AlertDialog(
            onDismissRequest = { complaintDialogOpen = false },
            title = { Text("Жалоба на рецепт") },
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    Text(recipe.title, color = Muted)
                    OutlinedTextField(
            ...
            confirmButton = {
                TextButton(
                    onClick = {
                        onSendComplaint(complaintTitle, complaintDescription)
                        complaintTitle = ""
            ...
            dismissButton = {
                TextButton(onClick = { complaintDialogOpen = false }) {
                    Text("Отмена")
                }
            }
        )
