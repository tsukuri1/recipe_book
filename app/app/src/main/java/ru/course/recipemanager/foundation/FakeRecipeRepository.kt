...
class FakeRecipeRepository(
    ...
    override suspend fun addIngredientsToShoppingList(recipeId: Long, ingredients: List<Ingredient>) {
    ...
    override suspend fun submitComplaint(recipeId: Long, recipeTitle: String, title: String, description: String) {
        require(title.isNotBlank()) { "Укажите причину жалобы" }
        require(description.isNotBlank()) { "Опишите проблему" }
        complaints += "$recipeId|$recipeTitle|$title|$description"
    }
    ...
    override suspend fun getAdminStats(): AdminStats = AdminStats(
        ...
        reportsCount = 1 + complaints.size
    )
}
