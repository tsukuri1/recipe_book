...
Команда поднимает сервисы `postgres` и `backend`. Backend через Docker доступен на `http://localhost:8082`.
...
PostgreSQL настраивается в `src/main/resources/application.yml`. Для Docker используются переменные окружения `DB_URL`, `DB_USERNAME`, `DB_PASSWORD`.
