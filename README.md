...
Система поддерживает роли пользователя и администратора. Пользователь создаёт и редактирует рецепты, работает с избранным, покупками, жалобами, профилем и настройками. Администратор модерирует рецепты, просматривает статистику и обрабатывает жалобы.
...
| **Документация** | Markdown, PNG-диаграммы |
...
```bash
git clone https://github.com/tsukuri1/recipe_book.git
cd recipe_book
```

### 2. Запуск backend через Docker

Docker Compose поднимает PostgreSQL и Spring Boot backend одной командой:

```bash
docker compose up --build
```

Backend будет доступен на `http://localhost:8082`, PostgreSQL из Docker — на `localhost:5433`.
Если порт `8080` свободен и нужен именно он, можно запустить так: `BACKEND_PORT=8080 docker compose up --build`.

### 3. Запуск backend без Docker
...
Для запуска без Docker PostgreSQL должен быть уже установлен локально, а база `ladushki` должна существовать.

Сервер без Docker запускается на `http://localhost:8080`.

Полезные адреса без Docker:

- Swagger UI: `http://localhost:8080/swagger-ui.html`
- OpenAPI JSON: `http://localhost:8080/v3/api-docs`
- REST API base URL: `http://localhost:8080/api`

Полезные адреса через Docker:

- Swagger UI: `http://localhost:8082/swagger-ui.html`
- OpenAPI JSON: `http://localhost:8082/v3/api-docs`
- REST API base URL: `http://localhost:8082/api`

### 4. Запуск мобильного клиента
...
Для Android-эмулятора Docker backend доступен по адресу `http://10.0.2.2:8082/api/`.

## Тестовые аккаунты

| Роль | Email | Пароль | Возможности |
|---|---|---|---|
| Пользователь | `user@ladushki.app` | `1234` | Каталог, избранное, покупки, жалобы, профиль, настройки |
...
| GET | `/api/favorites` | Получить избранные рецепты | USER |
...
| POST | `/api/complaints` | Отправить жалобу на рецепт | USER |
...
Полная документация API: [Swagger UI](http://localhost:8080/swagger-ui.html), [docs/09-api/openapi.yaml](docs/09-api/openapi.yaml), [docs/09-api/openapi-endpoints.md](docs/09-api/openapi-endpoints.md).  
Postman коллекция: [docs/09-api/postman-collection.json](docs/09-api/postman-collection.json).
...
| [09-api/](docs/09-api/) | REST endpoints, OpenAPI, Postman, проверка API |
...
| Метрика | Значение |
|---|---|
| Репозиторий | `https://github.com/tsukuri1/recipe_book` |
| Всего коммитов на момент фиксации статистики | 1 |
| Покрытие backend-тестами | 64.95% line coverage |
| Скриншоты GitHub Insights | Commit Activity и Punch Card |
...
Силина Оксана Романовна — разработка, тестирование и документация.  
Группа ПИЖ-б-о-23-2, траектория Mobile.

## Итоговые материалы для сдачи

| Файл | Назначение |
|---|---|
| [docs/13-final-report/Пояснительная_Записка.docx](docs/13-final-report/Пояснительная_Записка.docx) | Пояснительная записка |
| [docs/03-tables/course-tables.md](docs/03-tables/course-tables.md) | Таблицы для отчёта |
| [docs/09-api/openapi.yaml](docs/09-api/openapi.yaml) | OpenAPI-спецификация |
| [docs/09-api/postman-collection.json](docs/09-api/postman-collection.json) | Postman-коллекция |
...
- [Репозиторий проекта](https://github.com/tsukuri1/recipe_book)
- [Документация](docs/)
- [Android-клиент](app/)
